...
class CurrencyWidget : AppWidgetProvider() {
    ...
    //Called on default of every 30min
    ...
    //Called when removed from Home screen
    ...
    //Call update when broadcast received from service
    override fun onReceive(context: Context, intent: Intent) {
        super.onReceive(context, intent)
        val widgetID =  intent.getIntExtra("ID", 0)
        if (widgetID != 0) {
    ...
    companion object {
        ...
        internal fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager,
            ...
            //Scheduled JobIntentService enqueue
